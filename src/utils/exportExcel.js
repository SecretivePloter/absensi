...
export function exportMassStaffAttendance(recordsByStaff, periodLabel, filename = 'Rekap_Massal_Staff') {
  const headers = ['No', 'Tanggal', 'Jam Masuk', 'Jam Pulang', 'Alasan Pulang Awal', 'Lokasi', 'Metode', 'Catatan']
  const fmtTime = (t) =>
    t ? new Date(t).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : '-'

  const wb = XLSX.utils.book_new()

  // Untuk setiap staff yang ada, buatkan sheet baru
  recordsByStaff.forEach(staffData => {
    // Sanitize nama untuk menamai sheet (maks 31 karakter dan tanpa karakter terlarang excel: \ / ? * [ ])
    let sheetName = (staffData.staffName || 'Unknown').replace(/[\\/?*[\]]/g, '').trim()
    if (sheetName.length > 31) sheetName = sheetName.substring(0, 31)
    if (!sheetName) sheetName = 'Staff'

    const rows = staffData.records.map((r, i) => [
      i + 1,
      new Date(r.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }),
      fmtTime(r.check_in_at),
      fmtTime(r.check_out_at),
    ...
    const ws = buildStyledSheet({
      title: `Rekap Absensi - ${staffData.staffName}`,
      subtitle: `Periode: ${periodLabel} · ${exportStamp()} · ${staffData.records.length} catatan hadir`,
      headers,
      rows,
      centerCols: [0, 1, 2, 3, 6],
    })

    // Handle collision if there are duplicate names
  ...
  // Jika tidak ada data sama sekali (kosong), buat sheet dummy agar file tetap valid
  if (recordsByStaff.length === 0) {
    const ws = buildStyledSheet({
      title: 'Tidak Ada Data',
      subtitle: `Periode: ${periodLabel}`,
      headers: ['-'],
      rows: [['Tidak ada data rekam absen staff pada periode ini.']],
      centerCols: [],
    })
